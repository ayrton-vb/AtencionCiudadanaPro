...
@section('contenido')
    ...
    <section class="mt-5 d-flex flex-column justify-content-center align-items-center pt-5  text-center w-50 m-auto" id="intro">
        ...
        <div>
            <a data-aos="fade-up" href="/clientes/{{$tramite->categorias->id}}/tramiteByTramiteSucursal" type="button" class="btn btn-outline-danger  mx-2 fs-6 m-1  fw-bold">Volver</a>
                ...
                <div data-aos="flip-up" data-aos-delay="350" class="row pt-3 pb-3">
                    ...
                    <div class="col-10 pt-4 border-start border-danger  border-5 wrapper-tramite">
                        ...
                        @for ($i = 0; $i < $var2; $i++)
                                ...
                                <p id="{{$var[$i]->nombre}}" class="fw-bold redText">{{$var[$i]->nombre}}</p>


                            @foreach($requisitos as $requisito)
                                @if($requisito->id_tipoPersona == $var[$i]->id)
                                    <p class="">{{$requisito->dato}}</p>
                                @else
                                @endif
                            @endforeach
                      ...
                      {{--  @foreach($tipoPersonas as $tipoPersona)
                            @if(0 == 1)
                                <p id="{{$tipoPersona->nombre}}" class="fw-bold text-danger">{{$tipoPersona->nombre}}</p>
                            @else
                            @endif
